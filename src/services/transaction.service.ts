...
export class TransactionService {
   ...
   */
  async createWithDetails<T extends {date: string}, D extends {weight_kg: number; productId: number; personId: number}>(
   ...
   */
  async updateWithDetails<T extends {id: number; date?: string}, D extends {
    id?: number;
    weight_kg?: number;
    productId?: number;
    personId?: number;
    ...
    try {
      ...
      const result = await repository.dataSource.transaction(async (tx: any) => {
        ...
        const {details: _details, id: _id, ...cleanTransactionData} = transactionData;
        await repository.updateById(transactionData.id, {
          ...cleanTransactionData,
          total_kg: newTotalKg
        }, {transaction: tx});

        // 6. Retornar resultado con relaciones
        return repository.findById(transactionData.id, {
          include: [detailsRelationName]
        }, {transaction: tx});
      });
   ...
   */
  private validateDetailsForUpdate(details: Array<{
    id?: number;
    weight_kg?: number;
    productId?: number;
    personId?: number;
    ...
    for (const det of details) {
      if ((det.toUpdate ?? false) || (det.toCreate ?? false)) {
        if (!det.productId || !det.personId) {
          throw new HttpErrors.BadRequest('Product ID and Person ID are required for create/update operations');
        }
        if (!det.weight_kg || det.weight_kg <= 0) {
          throw new HttpErrors.BadRequest('Weight must be a positive number for create/update operations');
        }
      }
      if (det.toDelete && !det.id) {
        throw new HttpErrors.BadRequest('Detail ID is required for delete operations');
      }
      if (det.toUpdate && !det.id) {
        throw new HttpErrors.BadRequest('Detail ID is required for update operations');
      }
    }
  }
  ...
  public validateDate(date: string): void {
    ...
    if (year < 2000 || year > currentYear) {
      throw new HttpErrors.BadRequest(`Invalid date. Year must be between 2000 and ${currentYear}.`);
    }
  }

  private calculateTotalWeight(details?: Array<{weight_kg: number}>): number {
    return details?.reduce((sum, detail) => sum + (detail.weight_kg ?? 0), 0) ?? 0;
  }

  private createDetails(
    transactionData: {details?: Array<{weight_kg: number; productId: number; personId: number}>},
    transaction: {id: number},
    repository: any,
    detailsRelationName: string
  ): Promise<any>[] {
    return transactionData.details?.map(detail =>
